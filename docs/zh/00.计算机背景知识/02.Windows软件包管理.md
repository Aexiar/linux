...
| 优点                 | 描述                                                         |
| -------------------- | ------------------------------------------------------------ |
| `免管理员权限`       | 默认将软件安装在你的用户目录（`C:\Users\用户名\scoop`）下，安装时 不会弹出 UAC 账户控制提示。 |
| `不污染系统环境`     | 软件彼此隔离，所有的环境变量（ `PATH`）都由 Scoop 自动配置和管理，不会弄乱注册表。 |
| `真正的“绿色版”体验` | Scoop 倾向于下载解压即可用的便携版（Portable）软件。<br>卸载时直接删掉对应文件夹，不留任何垃圾。 |
| `极简版本切换`       | 可以同时安装 Java 8、Java 11 和 Java 17，用一行命令就能在它们之间无缝切换。 |
...
| 特性       | Scoop                                              | WinGet                                                |
| ---------- | -------------------------------------------------- | ----------------------------------------------------- |
| 官方背景   | `开源社区驱动`                                     | 微软官方出品（Win11 自带）                            |
| 核心定位   | `面向开发者`，偏向命令行工具、开源软件和绿色版软件 | `面向普通大众`，偏向传统 GUI 软件（QQ、微信、Office） |
| 安装方式   | 解压/便携化（Portable），安装在用户目录            | 运行官方的 `.exe` 或 `.msi` 安装包                    |
| 管理员权限 | `默认不需要`                                       | 大多数情况下需要（会弹出提示）                        |
| 环境变量   | `自动配置，且管理得井井有条`                       | 依赖软件安装包自带的配置，有时需要手动调              |
...
> [!NOTE]
>
> * ① 如果访问 GitHub 时遇到网络问题，请使用代理，如下所示：
>
> ```powershell
