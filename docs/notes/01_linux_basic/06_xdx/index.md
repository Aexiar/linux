...
| 特性         | 软链接                                                       | 硬链接                                                       |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 本质         | 文件系统中的特殊文件，指向另一个文件或目录                   | 文件系统中的特殊文件，直接指向另一个文件的 inode             |
| 存储方式     | 存储被链接的文件或目录的路径                                 | 与被链接的文件共享同一个 inode                               |
| 作用         | 当访问软链接时，系统会自动跳转到被链接的文件或目录           | 硬链接和被链接的文件共享同一个 inode，因此它们具有相同的设备号和 inode 号 |
...
* 对于文件夹（目录）来说，创建硬链接通常是不可能的，原因如下：
  * **目录结构的复杂性**：文件夹包含的是其他文件和文件夹的列表，这些列表是以特定的数据结构（如链表）存储的。硬链接通常指向文件的物理数据，而不是这种复杂的目录结构。
  * **目录的引用计数**：文件系统中的 inode 有一个引用计数，表示有多少个硬链接指向这个 inode。对于文件，增加一个硬链接会增加引用计数，删除一个硬链接会减少引用计数。对于目录，引用计数的逻辑更加复杂，因为目录下的文件可能被移动或删除，这会导致目录结构的变化，而硬链接不允许这种变化。
  * **避免循环引用**：硬链接可能会导致循环引用的问题。例如：如果允许创建目录的硬链接，可能会出现一个目录链接到自己的情况，这会导致无限循环，使得文件系统无法正确处理。
  * **安全性和一致性**：允许目录硬链接可能会破坏文件系统的安全性和一致性。例如：如果一个目录被删除，但是通过硬链接仍然可以访问其内容，这会导致文件系统的不一致。
* 硬链接不能跨分区的原因主要与文件系统的内部结构和设计有关：
  * **inode 表的局部性**：在文件系统中，每个文件都有一个对应的 inode，它包含了文件的元数据，如：文件大小、权限、时间戳等。inode 表是文件系统的一部分，它记录了文件系统中所有文件的元数据。当创建硬链接时，实际上是在文件系统中为同一个 inode 创建了一个新的目录入口。这意味着硬链接必须在同一个文件系统中，因为 inode 表是文件系统级别的，而不是跨文件系统共享的。
  * **文件系统的设计**：不同的分区可能使用不同的文件系统，如：ext4、NTFS、FAT32 等。这些文件系统可能有不同的设计和实现方式，它们之间的 inode 表是不兼容的。因此，一个文件系统无法直接引用另一个文件系统中的 inode。
  * **一致性和完整性**：硬链接的设计要求它指向的文件在文件系统中是唯一的。如果允许硬链接跨分区，那么这个文件的 inode 信息可能在另一个分区中不存在，这会导致文件系统的一致性和完整性问题。
  * **引用计数的维护**：硬链接的引用计数是在文件系统级别维护的。如果硬链接跨越了分区，那么引用计数的更新和管理将变得复杂，因为这需要跨文件系统进行协调，这在技术上是不可行的。
  * **安全性和隔离**：分区通常用于逻辑上隔离不同的数据集，以提高系统的安全性和稳定性。允许跨分区的硬链接可能会破坏这种隔离，增加系统安全风险。
...
>
> * ① 通常而言，软链接采用相对路径。
> * ② 删除软链接的时候，需要使用 `rm -rf xxx` ；否则，如果软链接链接的是目录，使用 `rm -rf xxx/` 会将对应目录中的内容删除！！！ 
...
* 在 Linux 系统中，设备可以分为两大类：`块设备`（block devices）和`字符设备`（character devices）。
  ...
  * 字符设备（Character Devices）：
    ...
    - 特点：字符设备`不支持`文件系统，它们通常用于处理流式数据，如：文本和二进制数据。性能相对较低，因为它们不能像块设备那样进行数据缓存。
