...
>   * 如果创建 6 个分区，可以划分 3 个主分区和 1 个扩展分区（其中扩展分区包含 3 个逻辑分区）。
>   * 如果创建 6 个分区，可以划分 2 个主分区和 1 个扩展分区（其中扩展分区包含 4 个逻辑分区）。
>   * 如果创建 6 个分区，可以划分 1 个主分区和 1 个扩展分区（其中扩展分区包含 5 个逻辑分区）。
> * ② GPT 分区不限制分区数量，即 GPT 分区没有主分区、扩展分区和逻辑分区之分。
> * ③ 目前，在安装操作系统的时候，大部分情况下都是选择 GPT 分区。
> * ④ 目前的 Win11 操作系统，已经强制使用 GPT 分区了；换言之，强制使用 UEFI 来引导操作系统。

### 1.4.2 格式化

* `分区`是将`磁盘`划分成`若干个`的`逻辑部分`，以便可以分别使用这些部分，即：

![image-20240205121655769](./assets/15.png)

* 而`格式化`是将`磁盘`或`分区的存储空间`组织成`文件系统`以便存储数据，即：

![image-20240205123253163](./assets/16.png)
...
![image-20240205140145497](./assets/18.png)
...
![image-20240205140745219](./assets/20.png)
...
![image-20240205145204394](./assets/24.png)
...
![image-20240205150708273](./assets/25.png)
...
![image-20240205150916796](./assets/26.png)
...
![image-20240205154330427](./assets/29.png)
...
![image-20240205154752071](./assets/30.png)
...
![image-20240205155656130](./assets/31.png)
...
![image-20240205160018805](./assets/32.png)
...
![image-20240205161933802](./assets/33.png)
...
![image-20240205161833798](./assets/34.png)
