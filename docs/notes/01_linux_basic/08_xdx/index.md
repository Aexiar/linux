...
> 注意⚠️： 因为在`普通模式`下，用户输入的`字符`会被解释为命令，而不是插入到文本中，所以一些用户习惯称其`普通模式`为`命令模式`。
...
* 示例：缓冲区存的如果为整行，则粘贴当前光标所在行的上方；否则，则粘贴至当前光标所在处的前面
...
* 如果在普通模式输入 `ctrl+v` ，则进入可视化模式是`面向快`，即 `-- VISUAL BLOCK --`。
