...
```shell
# 以字符串的形式；其中，u 表示所有者，user；g 表示所属组，group；o 表示其它人，other；a = u + g + o
...
> 注意⚠️：从上图中，我们可以得知，umask 是 `022` ，和文件的默认权限是 `644` ，而目录的默认权限是 `755` ，没有任何关系，到底怎么计算的？
...
> 温馨提示ℹ️：set uid ，即 suid  运行这个命令的时候，就相当于这个命令的`所有者`权限，用数字 `4` 表示。
...
> 温馨提示ℹ️：set gid ，即 sgid ，运行这个命令的时候，就相当于这个命令的`所属组（用户组）`权限，用数字 `2` 表示。
